...
static int eval(ASTNode *node, HashTable *db) {
    switch (node->kind) {
        ...
        case CMD_DEL: {
            int n = 0;
            for (ArgNode *a = node->args; a; a = a->next) n += ht_delete(db, a->value);
            printf("(integer) %d\n", n);
            break;
        }
        case CMD_EXISTS:
        ...
        case CMD_APPEND: {
            ...
            if (ex) {
                ...
                nv = malloc(len); snprintf(nv, len, "%s%s", ex, suf);
            } else { nv = strdup(suf); }
            ht_set(db, key, nv);
            printf("(integer) %zu\n", strlen(nv));
            break;
        ...
        case CMD_RENAME: {
            char *v = ht_get(db, arg(node,0));
            if (!v) { puts("ERR no such key"); break; }
        ...
        case CMD_MSET: {
            ArgNode *a = node->args;
            while (a && a->next) { ht_set(db, a->value, strdup(a->next->value)); a = a->next->next; }
            puts("OK");
            break;
        }
        case CMD_MGET: {
        ...
        case CMD_DBSIZE:  printf("(integer) %d\n", db->count); break;
        case CMD_FLUSHDB: {
            for (int i = 0; i < TABLE_SIZE; i++) {
